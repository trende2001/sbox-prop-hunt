...
public partial class Gun : Weapon
{
	...
	public virtual void ShootBullet( float damage = 0, float spread = 0, float force = 0, int count = 1 )
	{
		for ( int i = 0; i < count; i++ )
		{
			...
			var tr = Trace.Ray( Owner.AimRay.Position, Owner.AimRay.Position + (forward * 65565) ).Ignore( Owner ).WithAnyTags( "solid", "prop", "propplayer" ).WithoutTags( "trigger", "seeker" ).Run();
			if ( tr.Hit )
			{
				...
				if ( tr.Entity.IsValid() )
				{
					if ( tr.Hitbox.HasTag( "head" ) )
						damage *= 2;
					tr.Entity.TakeDamage( DamageInfo.FromBullet( tr.HitPosition, forward * force, damage ).WithWeapon( this ).WithAttacker( Owner ).WithTags( "propplayer", "prop", "solid" ) );
				}
